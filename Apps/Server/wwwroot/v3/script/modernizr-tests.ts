/**
 * @fileoverview Additional tests for modernizr.
 */

// Copied from https://codepen.io/davidgenetic/pen/FmHaD?editors=0010#0 and then slightly modified.
// Changes are:
//      * Called it autoplayaudio instead of autoplay
//      * Removed the logging of '[AUTOPLAY-ERROR]' to console
//      * Removed the bits that add CSS classes for autoplay

// Detect autoplay
// ---------------

// Credit goes to: http://mrcoles.com/detecting-html5-audio-autoplay/

// This script detects whether the current browser supports the
// autoplay feature for HTML5 Audio elements, and it sets the
// `AUTOPLAY` variable accordingly.

Modernizr.addTest('autoplayaudio', function() {

    // Audio file data URIs from comments in
    // [this gist](https://gist.github.com/westonruter/253174)
    // via [mudcube](https://github.com/mudcube)
    var mp3 = 'data:audio/mpeg;base64,';
    var ogg = 'data:audio/ogg;base64,';

    try {
        var audio = new Audio();
        var src = audio.canPlayType('audio/ogg') ? ogg : mp3;
        audio.autoplay = true;
        audio.volume = 0;

        // this will only be triggered if autoplay works
        audio.addEventListener('play', function() {
            Modernizr.autoplayaudio = true;
        }, false);

        audio.src = src;
    } catch(e) {
        //console.log('[AUTOPLAY-ERROR]', e);
    }

    return false;
});

// Returns true if the browser supports the WAV audio codec.
Modernizr.addTest('wavaudio', function() {
    var result = false;
    try {
        var audioElement = new Audio();
        result = audioElement.play !== undefined;
        if(result) {
            result = audioElement.canPlayType !== undefined;
        }
        if(result) {
            result = false;
            var testCodecs = [ 'audio/x-wav;codecs=1', 'audio/wav;codecs=1' ];
            var length = testCodecs.length;
            for(var i = 0;!result && i < length;++i) {
                var canPlay = audioElement.canPlayType(testCodecs[i]);
                result = canPlay && canPlay !== 'no';
            }
        }
    } catch(e) {
        result = false;
    }

    return result;
});